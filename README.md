...
Этот проект подходит для обучения и собеседования. Включает:

✅ **Правильная обработка ошибок** — все endpoints возвращают детальные ошибки  
✅ **Логирование** — видно все операции в логах  
✅ **Тестирование** — 4 теста покрывают основные сценарии  
✅ **Безопасность** — JWT, row-level locking, валидация  
✅ **Docker** — легко развернуть что угодно  
✅ **Документация** — Swagger UI по умолчанию доступен  

При собеседовании обратите внимание на:
- Как работает защита от race conditions (в `app/routers/bookings.py`)
- Как настроены тесты (в `tests/conftest.py`)
- Как работает JWT аутентификация (в `app/utils/security.py`)
- Как настроен Docker (в `Dockerfile` и `docker-compose.yml`)
